...
type UseSupabaseUploadOptions = {
  ...
  upsert?: boolean
  /**
   * Indicates the bucket is public and public URLs should be generated for successful uploads.
   * If true, each successful file will have a publicUrl derived via getPublicUrl.
   */
...
const useSupabaseUpload = (options: UseSupabaseUploadOptions) => {
  ...
  const [assets, setAssets] = useState<{ name: string; key: string; publicUrl?: string }[]>([])
  ...
  const onUpload = useCallback(async () => {
    ...
    const responses = await Promise.all(
      filesToUpload.map(async (file) => {
        ...
        if (error) {
          return { name: file.name, key: objectKey, message: error.message }
        } else {
          ...
          return { name: file.name, key: objectKey, publicUrl, message: undefined }
        }
      })
    )
  ...
  const responseErrors = responses.filter((x) => x.message !== undefined)
    ...
    setLoading(false)
    // Return latest assets snapshot (including ones just merged)
    return {
